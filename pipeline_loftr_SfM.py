...
def run(args):
    ...
    if model is None:
        logger.error("LoFTR SfM reconstruction failed.")
        if args.visualize and args.visualize_matches:
            plt.show()
        return None

    logger.info("Finished LoFTR SfM with statistics:\n%s", model.summary())

    if args.export_sparse_pcd:
        pcd_dir = args.sparse_pcd_dir or (outputs / "sparse_pcd_loftr")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--outputs",
        type=Path,
        default="outputs/botanic_garden_64_loftr",
    ...
    parser.add_argument(
        ...
        type=str,
        default="loftr",
        choices=list(match_dense.confs.keys()),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory for sparse colored .pcd files, default: <outputs>/sparse_pcd_loftr",
    )
    parser.add_argument(
        "--min_track_length",
        type=int,
