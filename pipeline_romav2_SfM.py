...
def run(args):
    ...
    if model is None:
        logger.error("RoMaV2 SfM reconstruction failed.")
        return None

    logger.info("Finished RoMaV2 SfM with statistics:\n%s", model.summary())

    if args.export_sparse_pcd:
        pcd_dir = args.sparse_pcd_dir or (outputs / "sparse_pcd_romav2")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--outputs",
        type=Path,
        default="outputs/sacre_coeur_romav2",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory for sparse colored .pcd files, default: <outputs>/sparse_pcd_romav2",
    )
    parser.add_argument(
        "--min_track_length",
        type=int,
